...
const TEAMS = {
  ...
  'H2': { id: 'H2', name: 'Chile', flagUrl: 'https://flagcdn.com/w40/cl.png', group: 'H' },
  ...
  // Grupo L
  'L1': { id: 'L1', name: 'Italia', flagUrl: 'https://flagcdn.com/w40/it.png', group: 'L' },
...
const MatchCard = ({ match, predictions, handleScoreChange, theme }) => {
...
export default function App() {
  ...
  const saveToFirebase = async () => {
    ...
    if (user && db) {
      try {
        const docRef = doc(db, 'artifacts', appId, 'prode_data', user.uid);
        await setDoc(docRef, { matches: predictions, updatedAt: new Date() }, { merge: true });
      } catch (e) {
        // Fallback
      }
    }
  ...
  const calculateStandings = useMemo(() => {
    ...
    Object.values(TEAMS).forEach(t => {
      standings[t.id] = { id: t.id, name: t.name, flagUrl: t.flagUrl, group: t.group, p: 0, w: 0, d: 0, l: 0, gf: 0, ga: 0, gd: 0, pts: 0 };
    });

    INITIAL_MATCHES.forEach(match => {
      const pred = predictions[match.id];
      if (pred && pred.t1 !== null && pred.t1 !== undefined && pred.t2 !== null && pred.t2 !== undefined) {
        const s1 = pred.t1;
        const s2 = pred.t2;
        const st1 = standings[match.t1];
        const st2 = standings[match.t2];
